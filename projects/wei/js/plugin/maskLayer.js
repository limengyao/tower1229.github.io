    ...
    !function () {
        var MaskLayer = function ($item, options) {
            ...
            if (this._$maskLayer = $item.addClass("u-maskLayer"), this.state = this._$maskLayer.is(".z-show") ? "show" : "hide", this.settings = $.extend({clickHideMode: 1, closeButton: !0, onShow: function () {
                ...
                theClass._$maskLayer.append($closeButton), $closeButton.on("click", function (e) {
                    e.preventDefault(), theClass.hide()
                })
            }
            "show" == this.state ? theClass.show("_init") : theClass.hide("_init"), this.settings.clickHideMode && (theClass._$maskLayer.on("click", function () {
                theClass.hide()
            }), 1 == theClass.settings.clickHideMode && theClass._$maskLayer.children().on("click", function (e) {
                e.stopPropagation()
            }))
        };
        MaskLayer.prototype.on = function (eventName, fn) {
            ...
            if (this._events[eventName])for (var callbacks = this._events[eventName], i = 0; i < callbacks.length; i++)callbacks[i]()
