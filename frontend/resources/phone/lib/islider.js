...
(function (global) {
    ...
    iSliderPrototype.bind = iSliderPrototype.delegate = function (evtType, selector, callback) {
        function handle(e) {
            ...
            //alert($(target).parents(selector).length)
            if($(target).is(selector) || !!$(target).parents(selector).length){
                if(!$(target).is(selector)){
                    target = $(target).parents(selector).get(0);
                }
                callback.call(target);
            }
        }
        this.wrap.addEventListener(evtType, handle, true);
    };
